...
const MagneticWord = ({ children }: { children: string }) => {
  ...
  return (
    <motion.span
      ...
      className="origin-center hover:text-black/60 transition-colors duration-300 cursor-crosshair"
    >
      {children}&nbsp;
    </motion.span>
  );
...
export default function AboutClient() {
  ...
  return (
    ...
    >
      ...
      <div className="w-full flex flex-col items-center justify-center text-center px-6 pt-24 pb-16 relative z-10">
        <motion.p 
        ...
        <h1 className={`${playfair.className} text-5xl md:text-7xl lg:text-8xl font-medium tracking-tight leading-[1.1] max-w-5xl flex flex-wrap justify-center text-black`}>
          {headline.map((word, i) => (
            <motion.div
              key={i}
      ...
      <section className="w-full py-20 px-6 md:px-12 relative z-20">
      ...
      <section className="w-full py-20 px-6 md:px-12 relative z-20 bg-[#fbfaf8]">
        <div className="max-w-[1300px] mx-auto grid grid-cols-1 md:grid-cols-2 gap-16 items-center">
          <motion.div 
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
